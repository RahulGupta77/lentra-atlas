...
import { get_lap_document_meta_data } from "../../../services/ParsedDataWindowService";
...
const ParsedDataWindow = ({ updateDocStatusTrigger }) => {
  ...
  useEffect(() => {
    const fetchDocumentsFromServer = async () => {
      const response = await get_lap_document_meta_data(id);
  ...
  useEffect(() => {
    ...
    // Bind to the parsed_data_lentra_poc event
    channel.bind("loan_against_property_direct_notification", (payload) => {
      try {
  ...
  return (
    <div className="parsed-data-window">
      <div className="parsed-data-window-header">
        <h2>Loan Against Property Documents</h2>
        <label
